...
class TaskProcessorImpl(val task: Task) : TaskProcessor {
  ...
  private val eduAIHintsProvider = EduAIHintsProvider.forCourse(task.course)
  ...
  private fun getFunctionSignatures(file: TaskFile, project: Project): List<FunctionSignature> {
    ...
    val functionSignatures = eduAIHintsProvider?.getFunctionSignatures(
    ...
    return task.taskFiles.values.filterNot { it.isTestFile }.flatMap { file ->
      getStringsIfFileUnchanged(file, project)?.let {
        ...
        runReadAction {
          ...
          val functionsToStringsMap = eduAIHintsProvider?.getFunctionsToStringsMap(psiFile) ?: return@runReadAction emptyList()
  ...
  override fun getShortFunctionFromSolutionIfRecommended(code: String, functionName: String): String? {
    ...
    if (signature != null && functionsSignaturesFromSolution.contains(signature)) {
      ...
      return runReadAction {
        ...
        eduAIHintsProvider?.getFunctionBySignature(
  ...
  private fun getFunctionSignaturesFromGeneratedCode(code: String, project: Project, language: Language) = runReadAction {
    ...
    eduAIHintsProvider?.getFunctionSignatures(psiFile, SignatureSource.GENERATED_SOLUTION)
  }
  ...
  override fun getModifiedFunctionNameInCodeHint(codeStr: String, codeHint: String) = runReadAction {
    ...
    val functionName = eduAIHintsProvider?.findChangedMethods(codePsiFile, codeHintPsiFile, true)?.firstOrNull()
  ...
  private fun getFunctionPsiWithName(code: String, functionName: String, project: Project, language: Language) = runReadAction {
    ...
    eduAIHintsProvider?.getFunctionBySignature(codePsiFile, functionName)
  }
  ...
  override fun applyCodeHint(codeHint: String): String? {
    ...
    val functionSignaturesFromCodeHint = runReadAction {
      eduAIHintsProvider?.getFunctionSignatures(
    ...
    for (newFunction in functionSignaturesFromCodeHint) {
      runReadAction { eduAIHintsProvider?.getFunctionBySignature(codeHintPsiFile, newFunction.name) }?.let { psiNewFunction ->
        WriteCommandAction.runWriteCommandAction(project, null, null, {
          eduAIHintsProvider?.getFunctionBySignature(psiFileCopy, newFunction.name)?.replace(psiNewFunction)?.let {
            isFileModified = true
          } ?: run {
  ...
  private fun findChangedFunctions(task: Task, project: Project) {
    ...
    for (taskFile in visibleTaskFiles) {
      ...
      val changedFunctions = eduAIHintsProvider?.findChangedMethods(beforePsiFile, afterPsiFile)
  ...
  private fun PsiFile.filterAllowedModifications(task: Task, project: Project, signatureSource: SignatureSource): String {
    findChangedFunctions(task, project)
    val functionSignatures = eduAIHintsProvider?.getFunctionSignatures(this, signatureSource) ?: return ""
    return runReadAction {
      functionSignatures.filter { functionSignature ->
        ...
        eduAIHintsProvider.getFunctionBySignature(this, it.name)?.text ?: ""
      }
    }
  }
  ...
  companion object {
    fun applyInspections(code: String, project: Project, language: Language): String {
      val inspections = runReadAction { project.course?.let { EduAIHintsProvider.forCourse(it)?.getInspections() } } ?: return code
