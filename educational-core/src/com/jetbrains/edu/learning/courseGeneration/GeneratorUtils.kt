...
object GeneratorUtils {
  ...
  private fun doCreateChildFile(
    ...
    fun writeBinary(contents: BinaryContents) {
      virtualTaskFile.setBinaryContent(contents.bytes)
    }

    fun writeTextual(contents: TextualContents) {
      val text = EduMacroUtils.expandMacrosForFile(holder, virtualTaskFile, contents.text)
      VfsUtil.saveText(virtualTaskFile, text)
    }

    when (fileContents) {
      is BinaryContents -> writeBinary(fileContents)
      is TextualContents -> writeTextual(fileContents)
      is UndeterminedContents -> if (virtualTaskFile.isToEncodeContent) { // fallback to the legacy way to interpret task file content
        writeBinary(fileContents.interpretAsBinary())
      }
      else {
        writeTextual(fileContents.interpretAsTextual())
      }
    }
