...
class StudyTaskManager(private val project: Project) : DumbAware, Disposable {
  ...
  var course: Course?
    ...
    set(course) {
      ...
      fun contentsHolderIsForThisFileFromThisStorage(eduFile: EduFile): Boolean {
      ...
      course?.visitEduFiles { eduFile ->
        ...
        if (contentsHolder !is StorageAuthorContentsHolder)

        if (!contentsHolderIsForThisFileFromThisStorage(eduFile)) {
